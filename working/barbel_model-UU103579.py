...
class FishEnvironment(pcrfw.DynamicModel, ):
    def __init__(self, input_dir, output, ut_array, dt_array, spatial_resolution, temporal_resolution, xmin, ymin, nrbarbels, spawning_conditions, adult_conditions, radius):
    ...
    def initial(self):
        ...
        # self.water.area.nearestforpidx10 = campo.spread (self.barbel.adults.movemode, self.water.area.zero, self.water.area.one) # find_nearest (self.barbel.adults, self.water.area, 9, self.water.area.swimmable, self.water.area.zero, self.water.area.one)
        # self.water.area.nearestforpidx10.is_dynamic = True
    ...
    def dynamic(self):
        ...
        self.water.area.connected_swimmable = campo_clump (self, self.water.area.swimmable)
        movingX, movingY, spawning_area, travel_distance, has_spawned, movemode = move (self, self.water.area.connected_swimmable, self.water.area.swimmable, self.water.area.spawning_grounds, self.barbel.adults, self.water.area, self.nrbarbels, self.currentTimeStep(), self.barbel.adults.has_spawned, radius) 
